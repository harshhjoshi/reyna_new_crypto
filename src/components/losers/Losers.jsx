...
import styles from "./losers.module.scss";
...
const Losers = () => {
  ...
  const GET_ALL_CRYPTO_CORENCIES = async () => {
    setLoading(true);
   await APICall.get(
      `${API.MARKET_LIST}order=market_cap_asc&per_page=1000&page=1&sparkline=false`
    )
      .then((response) => {
        console.log("Getting Data Loser", response.status);
